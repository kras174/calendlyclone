...
import { PublicEvent, getPublicEvents } from '@/server/actions/events';
...
export default function PublicProfile({ userId, fullName }: PublicProfileProps) {
  ...
  return (
    <div className="max-w-5xl mx-auto p-5">
      ...
      {/* Display user's name */}
      <div className="text-4xl md:text-5xl font-black mb-4 text-center">{fullName}</div>
      ...
      {events.length === 0 ? (
        <div className="text-center text-muted-foreground">No events available at the moment.</div>
      ) : (
